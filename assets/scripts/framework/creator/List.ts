 ...
 * 
 * 备注：
 * numItems 设置长度(修改此值刷新列表)
 * renderEvent 设置item的事件
 * selectedEvent 选择item的事件
 * displayItemNum 当前渲染item数
 * displayData 当前item数据
 * aniDelItem 删除item
 * scrollTo 滚动到
 * height 修改item高度 动态适配大小
 * skipPage 跳转页数相关
 * selectedId 设置选择item
 * updateItem 更新指定item
 * updateAll 更新所有
...
export default class List extends cc.Component {
    ...
    set customCallBack (cb) {
        this._customCallBack = cb
    }
    get customCallBack (): Function {
        return this._customCallBack
    }
    ...
    _getOutsideItem() {
        ...
        for (let n: number = this.content.childrenCount - 1; n >= 0; n--) {
            ...
            let has = false
            for (const v of this.displayData) {
                if (v.id == item._listId) {
                    has = true
                    break;
                }
            }
