...
import BookMonsterItem from "./BookMonsterItem";

const {ccclass, property} = cc._decorator;

@ccclass
export default class BookMonsterNode extends ListItem {
    @property(cc.Prefab)
    bookMonsterItem: cc.Prefab = null;
    ...
    setData (data: Array<BOOKOPENDATA>) {
        ...
        for (let i = 0; i < data.length; i++) {
            let item = cc.instantiate(this.bookMonsterItem)
            this.bookNode.addChild(item)

            let script = item.getComponent(BookMonsterItem)
